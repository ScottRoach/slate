...
var messages = {
  ...
  logTransferFailed: function logTransferFailed(errMsg) {
    gutil.log('Transfer Failed:', gutil.colors.yellow('' + (typeof errMsg === 'string' ? errMsg : 'File(s) failed to upload to store. See log notes in deploy.log')));
  },
  ...
  logDeploys: function logDeploys(cmd, files) {
    ...
    var action = cmd === 'deploy' ? 'added/changed ' : 'removed ';
  ...
  logDeployErrors: function logDeployErrors(cmd, files, err) {
    var timestamp = 'Deploy error @ ' + new Date() + '. ';
    var action = cmd === 'deploy' ? 'added/changed ' : 'removed ';
    var amount = files.length + ' file(s): ';
    var fileList = files.join(', ') + '.\n';
    var errMsg = err + ' \n';
